...
var enhanceHeader = require("./enhanceHeader");
...
function streamHtml(url, cache, options) {
	...
	let headers = enhanceHeader(options);

	console.log(`Calling ${url} with ${headers.Authorization}`);
	...
	var request = bhttp.get(url,  // TODO :: https://github.com/joepie91/node-bhttp/issues/3
		...
		.then(function (orgResponse) {
			...
			console.log("Side: " + response.url + " with StatusCode: " + response.statusCode)
